...
% break for weights conditioning
maxAcc = 0;
weights = [];
warning off
...
for g = 1:10000
    ...
    %Weights
    dPoints = dPoints * wPoints;
    dAssists = dAssists * wAssists;
    dSteals = dSteals * wSteals;
    dBlocks = dBlocks * wBlocks;
    dRebounds = dRebounds * wRebounds;
    ...
    for x = 1:1000
        ...
        tempMeanAcc = mean(accuracy);
        if tempMeanAcc > maxAcc
            maxAcc = tempMeanAcc;
            weights = [wPoints, wAssists, wSteals, wBlocks, wRebounds];
        end
    end
end
maxAcc
